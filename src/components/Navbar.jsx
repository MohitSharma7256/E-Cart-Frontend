...
export default function Navbar() {
  ...
  function getAPIData() {
    ...
    if (Array.isArray(CartStateData) && CartStateData.length) {
      ...
      // Filter cart items for current user - handle both string and object user fields
      const userCart = CartStateData.filter(x => {
        const itemUserId = typeof x.user === 'object' ? x.user?._id : x.user
        return itemUserId === userId
      })
      setCart(userCart)
      console.log('Cart Data:', userCart) // Debug logging
    } else {
      setCart([])
    }
  }
    ...
    <>
      <header className="top-header">
        <nav className="navbar navbar-expand-xl w-100 navbar-dark container gap-3">
          ...
          <a className="mobile-menu-btn d-inline d-xl-none" href="#" data-bs-toggle="offcanvas"
                ...
                return <div key={item._id}>
                  <div className="d-flex align-items-center gap-3">
                    ...
                    <div className="">
                      ...
                      <p className="mb-0"><strong>{item.quantity || item.qty} X &#8377;{product.finalPrice || item.price}</strong>
                      </p>
                    </div>
